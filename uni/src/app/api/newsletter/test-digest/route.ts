...
/**
 * GET /api/newsletter/test-digest?secret=xxx
 * Sends a test broadcast to the audience. Protected by CRON_SECRET query param.
...
export async function GET(request: NextRequest) {
  ...
  const topStories = allPosts.slice(0, 8);
  ...
  // Reuse the email builder from the digest cron
  const emailHtml = buildDigestEmail(topStories, now);

  const { data, error } = await resend.broadcasts.create({
    audienceId,
    from: 'uni-uk.ai Daily <digest@uni-uk.ai>',
    subject: `[TEST] What's Trending — ${dateStr}`,
    html: emailHtml,
    name: `Test Digest ${now.toISOString()}`,
  });

  if (error) {
    return NextResponse.json({ error: 'Broadcast create failed', details: error.message }, { status: 500 });
  }

  const sendResult = await resend.broadcasts.send(data!.id);

  if (sendResult.error) {
    return NextResponse.json({ error: 'Broadcast send failed', details: sendResult.error.message }, { status: 500 });
  }
    ...
    return `
      <tr>
        <td style="padding:${i === 0 ? '0' : '24px'} 0 24px 0;${i < stories.length - 1 ? 'border-bottom:1px solid #e5e5e5;' : ''}">
          ${imageBlock}
          <p style="margin:0 0 6px 0;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#e11d48;font-family:monospace;">
            ${story.category}
          </p>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f5f5f5;">
    <tr><td align="center" style="padding:32px 16px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;">
        <tr><td style="padding:32px 32px 24px 32px;border-bottom:3px solid #171717;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
            <td><h1 style="margin:0;font-size:24px;font-weight:900;font-family:Georgia,'Times New Roman',serif;color:#171717;">uni-uk<span style="color:#e11d48;">.ai</span></h1></td>
            <td align="right" style="vertical-align:bottom;"><span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#737373;font-family:monospace;">Daily Digest</span></td>
          </tr></table>
        </td></tr>
        <tr><td style="padding:24px 32px 8px 32px;">
          <p style="margin:0;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#737373;font-family:monospace;">${dateStr}</p>
          <h2 style="margin:8px 0 0 0;font-size:28px;font-weight:900;font-family:Georgia,'Times New Roman',serif;color:#171717;">What's Trending Today</h2>
        </td></tr>
        ...
        <tr><td style="padding:24px 32px;border-top:1px solid #e5e5e5;">
          <p style="margin:0 0 8px 0;font-size:12px;color:#737373;text-align:center;">You received this because you subscribed to the uni-uk.ai daily digest.</p>
          <p style="margin:0;font-size:12px;color:#737373;text-align:center;">
            ...
            &nbsp;&middot;&nbsp;<a href="https://uni-uk.ai/blog" style="color:#737373;text-decoration:underline;">View on web</a>
            &nbsp;&middot;&nbsp;<a href="https://uni-uk.ai/privacy" style="color:#737373;text-decoration:underline;">Privacy</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
