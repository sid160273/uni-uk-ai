...
/**
 * Generates a crypto market analysis story
 */
export async function generateCryptoStory(
  ...
  const marketContext = topCoins.slice(0, 10).map((coin, i) =>
    `${i + 1}. ${coin.name} (${coin.symbol.toUpperCase()}): ${formatPrice(coin.current_price)} — ${coin.price_change_percentage_24h > 0 ? '+' : ''}${coin.price_change_percentage_24h.toFixed(2)}% (24h)`
  ).join('\n');
...
WRITE:
- 500-800 words — concise, punchy
- Clear H2 (##) and H3 (###) headings
- Include specific price data and percentage changes
- "What's Moving" section with specific coins
...
3. 1-2 external links to authoritative crypto sources

OUTPUT FORMAT (JSON only, no markdown blocks):
{
  "title": "Engaging crypto headline (50-70 chars)",
  "excerpt": "Compelling summary (120-160 chars)",
  "content": "Full markdown content",
  "coins": ["BTC", "ETH", ...mentioned coins],
  "tags": ["tag1", "tag2", "tag3", "tag4", "tag5"]
}`;

  const userPrompt = `Write a crypto market analysis based on current data:
...
MARKET DIRECTION: ${marketTrend > 1 ? '📈 Bullish' : marketTrend < -1 ? '📉 Bearish' : '↔️ Sideways'} (avg top 5: ${marketTrend > 0 ? '+' : ''}${marketTrend.toFixed(2)}%)
